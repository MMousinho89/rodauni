...
$field = preg_replace('/[^a-zA-Z0-9_]/','', (string)($_GET['field'] ?? 'filial_id'));
...
$nameCol = hasCol($cols,'nome_filial') ? 'nome_filial' : (hasCol($cols,'nome') ? 'nome' : null);
if(!$nameCol){
  http_response_code(500);
  echo "Tabela {$table} precisa ter coluna nome_filial ou nome.";
  exit;
}

$sql = "SELECT id, {$nameCol} AS nome";
if(hasCol($cols,'ativo')) $sql .= ", ativo";
$sql .= " FROM {$table} WHERE 1=1 ";

$params = [];
if($q !== ''){
  $sql .= " AND (CAST(id AS CHAR) LIKE :q OR {$nameCol} LIKE :q) ";
  $params[':q'] = "%{$q}%";
}

$sql .= " ORDER BY {$nameCol} ASC LIMIT 500";
$st = $pdo->prepare($sql);
$st->execute($params);
...
<head>
  ...
  <title>RODAUNI — Lookup Filiais</title>
  ...
  <div class="container py-3">
    <div class="d-flex align-items-center justify-content-between mb-2">
      <div>
        ...
        <h5 class="mb-0"><i class="bi bi-diagram-3 me-1"></i>Filiais</h5>
      </div>
    ...
    <div class="card ru-card">
      <div class="card-body">
        <form class="row g-2 align-items-end mb-2" method="get">
          ...
          <div class="col-md-8">
            <label class="ru-field-label">Buscar (ID ou Nome)</label>
            <input type="text" class="form-control form-control-sm" name="q" value="<?= htmlspecialchars($q) ?>" placeholder="Ex.: 5 ou ITAPEVI">
          </div>
          <div class="col-md-2">
            ...
            <tbody id="tb">
              ...
              <?php else: ?>
                <?php foreach($rows as $r): ?>
                  <tr class="ru-row" data-id="<?= (int)$r['id'] ?>" data-label="<?= htmlspecialchars((string)$r['nome']) ?>">
                    <td><?= (int)$r['id'] ?></td>
                    <td><strong><?= htmlspecialchars((string)$r['nome']) ?></strong></td>
                    <?php if(hasCol($cols,'ativo')): ?>
...
(function(){
  ...
  // Enter confirma se tiver seleção
